...
export default function CreateTemplate() {
  ...
  useEffect(() => {
    const fetchTemplate = async () => {
      ...
      const response = await fetch(`https://6i4ntknht5.execute-api.us-east-1.amazonaws.com/staging/templates/${id}?userId=${UserId}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const UserId = await GetUserData();
    const Tasks = tasks.map(task => [task.name, task.duration]);
    try {
